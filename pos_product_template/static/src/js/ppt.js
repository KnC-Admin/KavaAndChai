    ...
    var VariantListWidget = PosBaseWidget.extend({
        ...
        init: function(parent, options) {
            ...
            this.click_variant_handler = function(event){
                ...
                else if(self.pos.config.allow_product_pack && variant.is_pack && variant.is_selective_pack){
                    self.gui.show_popup('selective-product-widget',{'product':variant,'edit_pack':false});
                }
                else{
                    self.__parentedParent.hide();
        ...
        render_variant: function(variant){
            var image_url = window.location.origin + '/web/image?model=product.product&field=image_medium&id=' + variant.id;
